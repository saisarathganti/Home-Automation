  ...
  if (user) {
	...
	firecnt.on('value', function(snapshot) {
		if(snapshot.val()){
			...
			window.open('mailto:saisarathganti@example.com?subject=FIRE IN THE HOUSE &body=PLEASE COME TO THE MENTIONED ADDRESS WITH FIRE TRUCKS');
		}
		else
...
function power(){
	var fireref=firebase.database().ref();
	fireref.child("users/"+peta+"/trp").set(1);
}
function volumeup(){
	var fireref=firebase.database().ref();
	fireref.child("users/"+peta+"/trpup").set(1);
}
function volumedown(){
	var fireref=firebase.database().ref();
	fireref.child("users/"+peta+"/trpdown").set(1);
}
function alarm(ab){
	var fireref=firebase.database().ref();
	if(ab==0){
